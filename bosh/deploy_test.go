...
var _ = Describe("Deploy", func() {
	...
	It("Deploys the director", func() {
		_, _, err := client.Deploy(nil, nil, false)
		Expect(err).ToNot(HaveOccurred())
		expectedCommand := fmt.Sprintf("Running bosh command: create-env %s/director.yml --state %s/director-state.json --vars-store %s/director-creds.yml --ops-file %s/extra_tags.yml --var tags={concourse-up-project: happymeal,concourse-up-component: director}", tempDir, tempDir, tempDir, tempDir)
	...
	Context("When tags are set", func() {
		It("Applies the tags ops file to the director manifest", func() {
			exampleConfig.Tags = []string{"key=value"}
			exampleConfig.TagsIsSet = true
			client = &Client{
				config:   exampleConfig,
				metadata: terraformMetadata,
				director: directorClient,
			...
			_, _, err := client.Deploy(nil, nil, false)
			Expect(err).ToNot(HaveOccurred())
			expectedCommand := fmt.Sprintf("Running bosh command: create-env %s/director.yml --state %s/director-state.json --vars-store %s/director-creds.yml --ops-file %s/extra_tags.yml --var tags={key: value,concourse-up-project: happymeal,concourse-up-component: director}", tempDir, tempDir, tempDir, tempDir)
			Expect(actions).To(ContainElement(expectedCommand))
		})
	})
})
