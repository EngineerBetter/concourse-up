...
func (client *Client) runSQL(sqlStr string) error {
	...
	sshClient, err := client.directorSSH()
	if err != nil {
		return err
	}
	...
	db, err := sql.Open("postgres+ssh", fmt.Sprintf(
		"user=%s password=%s dbname=%s host=%s port=%s sslmode=verify-full sslrootcert=%s",
		client.config.RDSUsername,
		client.config.RDSPassword,
		client.config.RDSDefaultDatabaseName,
		client.metadata.BoshDBAddress.Value,
		client.metadata.BoshDBPort.Value,
		tmpFile.Name(),
	))
	if err != nil {
		return err
