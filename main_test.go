...
var _ = Describe("concourse-up", func() {
	BeforeSuite(func() {
		...
		cliPath, err = Build("github.com/EngineerBetter/concourse-up", "-o", "ctwr")
	...
	It("displays usage instructions on --help", func() {
		...
		Expect(session.Out).To(Say("Control Tower for Concourse - A CLI tool to deploy Concourse CI"))
